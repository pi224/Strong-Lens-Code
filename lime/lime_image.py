...
def blockSegmentation(width, gridLength):
    segmentation = np.zeros((width, width))
    numSegments = int(math.ceil(width / gridLength))

    for i in range(width):
        for j in range(width):
            multiplier = i // gridLength
            adder = j // gridLength
            segmentation[i][j] = int((multiplier * numSegments) + adder)
...
class LimeImageExplainer(object):
    ...
    def explain_instance(self, image, classifier_fn, labels=(1,),
        ...
        try:
            # segments = segmentation_fn(image) original line
            # segments = quickshift(image[:, :, 0:3], ratio=0.2, kernel_size=4,
            #                             max_dist=200, random_seed=random_seed)
            segments = blockSegmentation(101, 15)
